...
const base = { account: { status: 'ACTIVE' as const }, profile, verification: { status: 'VERIFIED' as const, identityVerified: true, ageVerified: true, verifiedAt: null, expiresAt: null }, locations: [{ location: { active: true } }] as never[], media: [{ status: 'APPROVED', deleted_at: null }] as ProfileMedia[], hasEntitlement: true, canonicalEligible: true, dataAvailable: true }
...
describe('Velvet onboarding Step 06 readiness', () => {
  ...
  it.each([['KYC missing', { verification: null }], ['adult verification missing', { verification: { ...base.verification, ageVerified: false } }], ['service area missing', { locations: [] }], ['profile incomplete', { profile: { ...profile, bio: null, status: 'DRAFT' as const } }], ['billing missing', { hasEntitlement: false }]])('blocks %s', (_label, override) => { const result = buildPublicationReadiness({ ...base, ...override, canonicalEligible: false }); expect(result.items.some((item) => !item.ready)).toBe(true) })
  it.each(['PENDING_MODERATION', 'REJECTED', 'QUARANTINED'] as const)('does not satisfy media with %s', (status) => { const result = buildPublicationReadiness({ ...base, media: [{ status, deleted_at: null } as ProfileMedia], canonicalEligible: false }); expect(result.items.find((item) => item.key === 'photos')?.ready).toBe(false) })
  it('keeps text moderation separate from approved media', () => { const result = buildPublicationReadiness({ ...base, profile: { ...profile, content_moderation_status: 'PENDING' }, canonicalEligible: false }); expect(result.items.find((item) => item.key === 'photos')?.ready).toBe(true); expect(result.items.find((item) => item.key === 'publication')?.detail).toContain('texto') })
  it('fails closed on unavailable data', () => { const result = buildPublicationReadiness({ ...base, dataAvailable: false, canonicalEligible: false }); expect(result.items.find((item) => item.key === 'publication')?.ready).toBe(false) })
  it('derives ownership from auth and rechecks the canonical view in the server action', () => {
    ...
    expect(action.match(/isProfileCanonicallyEligible/g)?.length).toBeGreaterThanOrEqual(3)
