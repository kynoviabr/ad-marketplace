...
VALUES ('profile-videos', 'profile-videos', false, 52428800, ARRAY['video/mp4', 'video/webm', 'image/jpeg', 'image/webp'])
...
CREATE OR REPLACE FUNCTION public.enforce_profile_video_limit() RETURNS trigger
...
CREATE OR REPLACE FUNCTION public.reorder_profile_videos(p_profile_id uuid, p_video_ids uuid[]) RETURNS void
...
-- Storage remains private and inaccessible directly; all URLs are short-lived and server-authorized.
DROP POLICY IF EXISTS profile_videos_storage_select ON storage.objects;
DROP POLICY IF EXISTS profile_videos_storage_insert ON storage.objects;
