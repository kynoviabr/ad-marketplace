...
export function PublicComplianceLayer({ initialAgeAccepted, initialAnalyticsConsent }: { initialAgeAccepted: boolean; initialAnalyticsConsent: boolean | null }) {
  ...
  const isExcluded = EXCLUDED_PATHS.some((path) => logicalPath === path || logicalPath.startsWith(`${path}/`))
  ...
  const modalOpen = preferences || (!isExcluded && (ageOpen || consentOpen))
  ...
  const acceptAge = () => {
    setCookie(AGE_COOKIE, 'confirmed-v1')
    setAgeOpen(false)
    if (!readClientConsent()) setConsentOpen(true)
  }
  ...
  return (
    <div className="velvet-compliance-backdrop" role="presentation">
      <div ref={dialogRef} className="velvet-compliance-dialog" role="dialog" aria-modal="true" aria-labelledby="compliance-title">
        {ageOpen ? (
          <div className="velvet-age-gate-content">
            <div className="velvet-age-gate-header">
              <div className="velvet-age-gate-brand" aria-hidden="true">
            ...
            <p className="velvet-compliance-description">
              {en
                ? 'This website contains adult-oriented professional profiles. You must be at least 18 years old to continue.'
                : 'Este site apresenta perfis profissionais voltados ao público adulto. Você precisa ter 18 anos ou mais para continuar.'}
            </p>
            <div className="velvet-compliance-actions">
              <button type="button" className="velvet-compliance-primary" onClick={acceptAge}>
                {en ? 'I am 18 or older' : 'Tenho 18 anos ou mais'}
              </button>
              <Link className="velvet-compliance-secondary" href={localizePathname('/acesso-restrito', locale)}>
                {en ? 'Leave' : 'Sair'}
              </Link>
            </div>
            <nav className="velvet-compliance-support-nav" aria-label={en ? 'Legal information' : 'Informações legais'}>
              <Link
                href={localizePathname('/cookies', locale)}
                className="velvet-compliance-support-link"
              >
                {en ? 'Cookie Policy' : 'Política de Cookies'}
              </Link>
              <span className="velvet-compliance-support-divider" aria-hidden="true">·</span>
              <Link
                href={localizePathname('/termos', locale)}
                className="velvet-compliance-support-link"
              >
                {en ? 'Terms' : 'Termos'}
              </Link>
              <span className="velvet-compliance-support-divider" aria-hidden="true">·</span>
              <Link
                href={localizePathname('/privacidade', locale)}
                className="velvet-compliance-support-link"
              >
          ...
          <>
            ...
            <p className="velvet-compliance-description">{en ? 'We use necessary storage for security and operation. Optional first-party analytics only runs with your permission. No marketing trackers are installed.' : 'Usamos armazenamento necessário para segurança e funcionamento. Analytics próprios opcionais só funcionam com sua permissão. Não há rastreadores de marketing instalados.'}</p>
            {preferences && (
              <div className="velvet-consent-categories">
            ...
            <Link className="velvet-compliance-policy" href={localizePathname('/cookies', locale)}>{en ? 'Read the Cookie Policy' : 'Ler a Política de Cookies'}</Link>
          </>
        )}
      </div>
    </div>
