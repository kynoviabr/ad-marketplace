...
export async function PublicProfileGrid({ profiles }: PublicProfileGridProps) {
  ...
  return (
    <section className="velvet-home-profiles" aria-labelledby="featured-profiles-title">
      <header className="velvet-home-section-head"><div><p className="velvet-overline">{t('home.featured')}</p><h2 id="featured-profiles-title">{t('home.profilesToMeet')}</h2></div><Link href="/sao-paulo">{t('home.viewAll')} <span>→</span></Link></header>
