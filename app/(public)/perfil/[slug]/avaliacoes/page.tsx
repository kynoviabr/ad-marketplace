...
export default async function AllReviewsPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ page?: string }> }) {
  ...
  return <div className="reviews-page profile-detail-wrap">
    ...
    <p className="velvet-disclaimer velvet-disclaimer--reviews">{en ? 'Reviews reflect the opinions of registered members and do not constitute a guarantee by Velvet regarding services, conduct, or meetings.' : 'As avaliações refletem a opinião de membros cadastrados e não constituem garantia da Velvet sobre serviços, conduta ou encontros.'}</p>
