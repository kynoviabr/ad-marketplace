...
export async function moderateVideoAction(input: {
  videoId: string
  ...
  try {
    ...
    if (!/^[0-9a-f-]{36}$/i.test(input.videoId) || !['APPROVE', 'REJECT'].includes(input.decision) || (input.reason?.length ?? 0) > 1000) {
      return { success: false, error: 'Dados inválidos' }
    }
    if (input.decision === 'REJECT' && !input.reason?.trim()) {
      return { success: false, error: 'Informe o motivo' }
    }
    const admin = createAdminClient()
    const { error } = await admin.rpc('admin_moderate_video', {
      p_actor_account_user_id: moderator.id,
      p_video_id: input.videoId,
      p_decision: input.decision,
      p_reason: input.reason?.trim() || null,
    })
    if (error) {
      return { success: false, error: error.message || 'Vídeo não está pendente' }
    }
    revalidatePath('/admin/moderation')
    return { success: true }
  } catch {
    return { success: false, error: 'Falha ao moderar vídeo' }
  }
}
